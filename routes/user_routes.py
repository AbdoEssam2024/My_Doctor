...
from model.user_requests import LoginRequest, RegisterRequest, UpdateRequest, VerifyRequest
...
async def update_user_info(user_id: int, request: UpdateRequest):
    ...
    return update_user(user_id, request.username, request.password, request.email)
...
@router.post("/verify_user/{user_id}", dependencies=[Depends(verify_token)])
async def verify_user_info(user_id: int, code: VerifyRequest):
    return verify_user(user_id, code.code)
